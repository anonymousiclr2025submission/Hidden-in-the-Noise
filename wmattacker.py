...
class DiffWMAttacker():
    ...
    def attack(self, image_paths, out_paths, return_latents=True, return_dist=False):
        with torch.no_grad():
            ...
            for (img_path, out_path), prompt in tqdm(zip(zip(image_paths, out_paths), prompts)):
                ...
                latents = latents.sample(generator) * self.pipe.vae_scale_factor
                ...
                if len(latents_buf) == self.BATCH_SIZE:
                    image = batched_attack(latents_buf, prompts_buf, outs_buf)
                    latents_buf = []
                    prompts_buf = []
                    outs_buf = []
                if return_latents:
